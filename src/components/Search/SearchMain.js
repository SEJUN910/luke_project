...
function SearchMain(props) {
    ...
    const GoInfoMain = (user_n, user_id) => {
        ...
        let box = [];
        box = JSON.parse(localStorage.getItem("searchLists")) || [];
        let searchList = {
            "user_n" : user_n,
            "user_id": user_id
        }
        box.push(searchList);
        localStorage.setItem("searchLists", JSON.stringify(box));}
        ...
        <Grid item xs={12}>
            <Paper elevation={9} className="p-3 miniBox sortCS">
                <div className="d-flex flex-row align-items-center justify-content-between border-bottom pb-2 ms-1">
                    <div className="fw-bolder">회원검색</div>
                ...
                <div className="d-flex flex-row gap-2 mt-4">
                    <Form.Control type="text" placeholder="검색입력" onChange={searchText} value={search} onKeyPress={(e)=>{if(e.key === 'Enter'){setOpen(true); setStartSearch(new Date());}}}/>
                    <Button variant="contained" className="px-2 py-1" size="sm" onClick={()=>{setOpen(true); setStartSearch(new Date());}}>검색</Button>
                </div>
            </Paper>
        </Grid>
        <Dialog onClose={()=> {setOpen(false); setFrom('home')}} open={open} fullWidth={true} maxWidth="xl">
            <Grid item xs={12}>
                <div className="p-3 vh90">
                    <div className="d-flex flex-row align-items-center justify-content-between border-bottom pb-2 ms-1">
                        <div>회원검색</div>
                    ...
                    <div className="d-flex flex-row gap-2 my-3 w-25">
